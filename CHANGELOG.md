...
- **Breaking** for code importing navigate as a library: `Node` loses
  `command_references`, `add_command_reference` and
  `clear_command_references`; the parser holds the command queue, and
  `find_unreachable` takes it as a fourth argument. Simulation results are
  unchanged.
