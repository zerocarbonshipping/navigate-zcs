...
def plot_regulation_offsetting_expenses(manager, directory):
    ...
    for regulation_name, regulation in regulations.items():
        ...
        offsetting_expenses = profile.get_offsetting_expenses()
        flexibility_expenses = profile.get_flexibility_expenses()
        remedial_expenses = profile.get_remedial_expenses()

        total = offsetting_expenses + flexibility_expenses + remedial_expenses
        ...
        if np.any(offsetting_expenses > 0.):
            values.append(offsetting_expenses / divisor)
            labels.append('Offsetting')
            colors.append(CENTER_COLORS_BLUE[3])

        if np.any(flexibility_expenses > 0.):
            values.append(flexibility_expenses / divisor)
            labels.append('Flexibility')
            colors.append(CENTER_COLORS_YELLOW[3])

        if np.any(remedial_expenses > 0.):
            values.append(remedial_expenses / divisor)
        ...
        ax.set_ylabel('Non-compliance expenses [{}USD/year]'.format(prefix))
        ax.set_ylim([0., None])
        ax.grid(True, lw=0.3, alpha=0.5)
        format_axes(ax, 1, dateline, legend)

        save_figure(fig, directory, 'regulation_offsetting_expenses_{}.png'.format(regulation_name))
