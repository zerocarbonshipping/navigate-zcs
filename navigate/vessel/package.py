...
from navigate.investment.flows import Component, add_capex_flow, add_fixed_opex, get_flow_residual
...
def preprocess_packages(packages: list[Package],
    ...
    For each package:
      ...
      - Builds and stores a cumulative Component.
    ...
    packages[0].set_component(empty_component)
    ...
    for i, technology in enumerate(last_package.technologies):
        ...
        pkg = packages[i + 1]
        pkg.set_component(new_cumulative)
...
def npv_for_retrofit_steps(pkg_idx: int,
    ...
    for step in range(1, max_steps):
        new_pkg_idx = pkg_idx + step
        inc_cost_flow = packages[new_pkg_idx].cost_flow - packages[pkg_idx].cost_flow
        inc_cost_flow = _trim_flow_to_life(inc_cost_flow, remaining)
