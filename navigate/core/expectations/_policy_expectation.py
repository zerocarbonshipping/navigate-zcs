...
class _PolicyExpectation(_Expectation):
    def __init__(self):
        ...
        self._global_warming_potential: dict[
            str, float
        ] = {}  # dict[emission_name: float], global warming potential
    ...
    def set_expected_wtt(
        self, idx: int, key: tuple[str, ...], wtt: float | np.ndarray
    ) -> None:
        self._expected_wtt.setdefault(key, self._default_array())
        self._expected_wtt[key][idx:] = wtt

    def set_existing_wtt(
        self, idx: int, key: tuple[str, ...], wtt: float | np.ndarray
    ) -> None:
    ...
    def set_ttw_consumption(
        self, idx: int, key: tuple[str, ...], ttw: float | np.ndarray
    ) -> None:
    ...
    def set_expected_coefficient(
        self, idx: int, key: tuple[str, ...], coefficient: float | np.ndarray
    ) -> None:
