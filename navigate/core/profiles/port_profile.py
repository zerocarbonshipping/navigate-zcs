...
class PortProfile(_InfrastructureAggregateProfile):
    def __init__(self):
        ...
        self._TTW: dict[tuple[str, str], float] = {}
    ...
    def initialize(self, timeline: np.ndarray, emissions: dict[str, Emission],
        ...
        for emission_name, emission in emissions.items():
            ...
            for fuel_name, fuel in fuels.items():
                self._TTW[(fuel_name, emission_name)] = fuel.get_TTW(emission_name).get()
    ...
    def _get_fuel_emissions(self) -> KeysView[tuple[str, str]]:
        return self._bunker_WTT.keys()

    # bunker_WTT and bunker_WTW base getters generated by factory — see _container_util.py.
    # Intensity methods remain hand-written due to per-fuel LHV pattern.
...
# PortProfile bunker emission getter registrations (replaces ~12 hand-written methods)
register_tuple_dict_getters(PortProfile, 'bunker_WTT', gwp_attr='_GWP', data_attr='_bunker_WTT')
register_tuple_dict_getters(PortProfile, 'bunker_WTW', gwp_attr='_GWP', source_attrs=['_bunker_WTT', '_TTW'])
