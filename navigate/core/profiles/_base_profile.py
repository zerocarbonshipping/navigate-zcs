...
class _BaseProfile:
    ...
    def _default_array(
        self, default: float | bool | None = None, idx: int | slice = np.s_[:]
    ) -> np.ndarray:
    ...
    def _reset_dict(
        self, dict_: dict[str, np.ndarray], idx: int, default: float | None = None
    ) -> None:
        for array in dict_.values():
            array[idx:] = self._default_array(default, np.s_[idx:])

    def _reset_tuple_dict(
        self,
        dict_: dict[tuple[str, str], np.ndarray],
        idx: int,
    ...
    def _to_cumulative_dict(
        self, values: dict[str, np.ndarray]
    ) -> dict[str, np.ndarray]:
        return {key: self._to_cumulative(value) for key, value in values.items()}

    def _to_cumulative_any(
        self, value: np.ndarray | dict[str, np.ndarray]
    ...
    @staticmethod
    def _extract_multiply_nested_dict(
        result: dict[str, dict[str, np.ndarray]],
        mult_dict: dict[str, float],
        key: str | None = None,
        idx: int | slice = np.s_[:],
        ...
        if key:
            ...
            return {
                key: multiply_dicts(slice_dict(value, idx), mult_dict)
