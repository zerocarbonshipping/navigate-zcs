...
class _StepRate:
    ...
    def get(self, time):
        return np.where(
            np.asarray(time, dtype=float) >= self._step_day, self._late, self._early
        )
...
def _make_plant(
    ...
    plant.region.transport_cost["truck"] = (
        Scalar(COST_RATE) if cost_rate is None else cost_rate
    )
...
class TestCalculatePlantLogisticsExpectations:
    ...
    def test_constant_rate_levelizes_to_itself(self):
        ...
        assert np.allclose(
            plant.expectation.get_levelized_delivery_cost("port_a"),
            COST_RATE * DISTANCE,
        )

        assert np.allclose(
            plant.expectation.get_delivery_wtt("port_a", "carbon_dioxide"),
            WTT_RATE * DISTANCE,
        )
    ...
    def test_time_varying_rate_averages_over_operating_window(self):
        ...
        expected = np.array([5.0 / 3.0, 7.0 / 3.0, 3.0]) * DISTANCE
        assert np.allclose(
            plant.expectation.get_levelized_delivery_cost("port_a"), expected
        )
    ...
    def test_lead_time_is_read_per_step(self):
        ...
        expected = np.array([5.0 / 3.0, 3.0, 3.0]) * DISTANCE
        assert np.allclose(
            plant.expectation.get_levelized_delivery_cost("port_a"), expected
        )
    ...
    def test_plants_sharing_a_rate_keep_their_own_distance(self):
        ...
        assert np.allclose(
            far.expectation.get_levelized_delivery_cost("port_a"),
