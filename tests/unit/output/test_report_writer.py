...
class TestPrepareExport:
    ...
    def test_unmatched_name_warns_and_skips(self, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            export = _prepare_export(nodes, {'ghost': _node_report()}, 'output', 'Vessels')
...
class TestWriteReportErrorContainment:
    ...
    def _report_and_manager():
        ...
        manager = SimpleNamespace(name='global', nodes=Nodes(fleets={'fleet': None},
                                                             vessels={'vessel': None}))
        return report, manager

    def test_failed_xlsx_sheet_is_skipped_and_the_report_still_saves(self, tmp_path, caplog, monkeypatch):
    ...
    def test_failed_csv_sheet_is_skipped_and_the_report_still_saves(self, tmp_path, caplog, monkeypatch):
        report, manager = self._report_and_manager()
        ...
        def fail_on_fleets(sheet_name, nodes, extraction_dict, report_name, csv_data):
            if sheet_name == 'Fleets':
                raise RuntimeError('boom')
            csv_data[sheet_name] = {'headers': ['marker'], 'columns': [np.array([1., 2.])]}
