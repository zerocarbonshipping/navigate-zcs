...
LEVY_STAR = LEVY_DECK.format(
    name="levy", jurisdiction='[Port("port")]', extra='set_include_vessel("*", TRUE)'
)
LEVY_GHOST = LEVY_DECK.format(
    name="levy",
...
"""
    ...
    def test_wildcard_jurisdiction_scrubbed_to_surviving_ports(self, tmp_path, caplog):
        ...
        define_extra = (
            LEVY_DECK.format(name="levy", jurisdiction='Port("*")', extra="")
            + GHOST_ROUTE
        )
...
class TestActivationEdges:
    ...
    def test_every_port_reference_site_is_classified(self):
        ...
        found = set()
        for path in list((core / "nodes").glob("*.py")) + list(
