...
def market_shares(fleet):
    ...
    assert np.all(total > 0.0), "Deck validity: the fleet must never be empty"
    shares = {
        fuel_type: np.zeros_like(total, dtype=float)
        for fuel_type in fuel_types.values()
    }
...
class TestNoIncentive:
    ...
    def test_methanol_share_marginal(self, market_shares):
        assert np.all(
            market_shares[FuelTypeID.METHANOL] <= MAX_METHANOL_SHARE + EPS_SHARE
        )

    def test_ammonia_share_marginal(self, market_shares):
        assert np.all(
            market_shares[FuelTypeID.AMMONIA] <= MAX_AMMONIA_SHARE + EPS_SHARE
        )
