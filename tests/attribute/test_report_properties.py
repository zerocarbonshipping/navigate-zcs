...
PROFILE_CLASSES = {'add_property':            ManagerProfile,
...
_PROPERTY_CALL = re.compile(r'\b({})\(\s*(?:"[^"]*"\s*,\s*)?([A-Za-z][A-Za-z0-9]*)'
                            .format('|'.join(PROFILE_CLASSES)))
...
def _deck_properties():
    ...
    return [pytest.param(command, token, id='{}:{}'.format(path.relative_to(REPO_ROOT), token))
...
def test_deck_report_properties_resolve(command, token):
    ...
    assert hasattr(profile_class, getter_name), \
        "'{}' does not resolve: {} has no getter '{}'".format(token, profile_class.__name__, getter_name)

    parameters = list(inspect.signature(getattr(profile_class, getter_name)).parameters.values())[1:]
    assert all(p.default is not inspect.Parameter.empty
               ...
               for p in parameters), \
        "'{}' resolves to {}.{}, which the report writer cannot call without arguments".format(
            token, profile_class.__name__, getter_name)
